<template>
    <div>
      ============================84、决策树图表============================
      <div ref="chartDv" style="width: 100%;height: 500px"></div>
    </div>
</template>
    ...
    export default {
      ...
      methods:{
        initEchart(){
          ...
          let object = JSON.parse(this.chartDataStr);
          console.log(object);

          echart.setOption({
            tooltip: {                  // 提示框组件
              show:false,               //是否显示提示框
              trigger: 'item',          //触发类型设置为数据项图形触发
              triggerOn: 'mousemove'    //表示鼠标移动时触发提示
            },
            series:[
              {
                type: 'tree',           //本系列的图表类型设置为树图
                data: [object],         //树图数据
                left: '2%',
                right: '2%',
                top: '8%',
                bottom: '20%',
                symbol: 'emptyCircle',  //标记图形设置为空圆圈
                symbolSize:7,          // 标记的大小;默认是7
                orient: 'vertical',     // 树图的布局方向为垂直布局
                expandAndCollapse: true,//子树折叠与展开的交互，true表示展开部分，false表示展开所有
                label: {                //每个节点所对应的文本标签样式
                  normal: {
                    show:false,               //是否显示每个节点的文本标签
                    position: 'top',
                    rotate: -90,
                ...
                leaves: {                     //叶子节点的特殊配置
                  label: {                    //叶子节点所对应的文本标签样式
                    normal: {
                      show:false,             //是否显示每个子节点的的文本标签
                      position: 'bottom',     //标签显示的位置
                      rotate: -90,            //标签旋转角度；从 -90 度到 90 度。正值是逆时针。 -90表示垂直显示标签文字
                      verticalAlign: 'middle',  //文字垂直对齐方式;middle表示居中
                      align: 'left'             //文字水平对齐方式
                    }
                  }
                },
                lineStyle: {                  //线的样式
                  curveness:0                 //显示的曲度；0表示直线；默认为0.5；
                },
